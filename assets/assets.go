...
func UpsertAssetTypeAttribute(connection db.Connection, attribute AssetTypeAttribute) error {
	return db.Exec(connection,
		"insert into public.attribute_schema (asset_type, attribute_type, attribute, subtype, enable, translation, unit) "+
		...
		attribute.AssetTypeId, attribute.AttributeType, attribute.Id, string(attribute.Subtype), attribute.Enable, attribute.Translation, db.EmptyStringIsNull(attribute.Unit))
}
...
func UpsertAsset(connection db.Connection, asset Asset) (int, error) {
	assetId, err := db.QuerySingleRow[int](connection,
		"with asset_id as ("+
			"insert into public.asset (proj_id, gai, name, asset_type, lat, lon, description, tags) "+
		...
		asset.ProjectId, asset.GlobalAssetIdentifier, asset.Name, asset.AssetTypeId, db.EmptyFloatIsNull(asset.Latitude), db.EmptyFloatIsNull(asset.Longitude),
		db.EmptyStringIsNull(asset.Description), asset.Tags)
	return assetId, err
}
