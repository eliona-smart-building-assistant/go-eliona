...
func appRegistered(appName string) bool {
	app, _, err := client.NewClient().AppApi.GetAppByName(context.Background(), appName).Execute()
...
func registerApp(appName string) error {
	_, err := client.NewClient().AppApi.RegisterAppByName(context.Background(), appName).Execute()
	return err
}
...
func patchApplied(appName string, patchName string) bool {
	patch, _, err := client.NewClient().AppApi.GetPatchByName(context.Background(), appName, patchName).Execute()
...
func applyPatch(appName string, patchName string) error {
	_, err := client.NewClient().AppApi.ApplyPatchByName(context.Background(), appName, patchName).Execute()
	return err
}
